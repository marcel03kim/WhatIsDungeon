...
{   
    public MergeSlot[] slots;                                //게임 컴트롤러에서는 Slot 배열을 관리

    private Vector3 _target;
    private ItemInfo carryingItem;                      //잡고 있는 아이템 정보 값 관리

    private Dictionary<int, MergeSlot> slotDictionary;       //Slot id, Slot class 관리하기 위한 자료구조


    private void Start()
    {
        slotDictionary = new Dictionary<int, MergeSlot>();   //초기화
    ...
    void SendRayCast()
    {
        ...
        {
            var slot = hit.transform.GetComponent<MergeSlot>();          //Raycast를 통해 나온 Slot칸
            if (slot.state == MergeSlot.SLOTSTATE.FULL && carryingItem == null)
            {
            ...
            else if (slot.state == MergeSlot.SLOTSTATE.EMPTY && carryingItem != null)
            {//빈 슬롯에 아이템 배치
                slot.CreateItem(carryingItem.itemId);       //잡고 있는것 슬롯 위치에 생성
                Destroy(carryingItem.gameObject);           //잡고 있는것 파괴
            }
            else if (slot.state == MergeSlot.SLOTSTATE.FULL && carryingItem != null)
            {//Checking 후 병합
    ...
    {//랜덤한 슬롯에 아이템 배치
        ...
        while(slot.state == MergeSlot.SLOTSTATE.FULL)
        {
    ...
    {//모든 슬롯이 채워져 있는지 확인
        ...
        {
            if (slot.state == MergeSlot.SLOTSTATE.EMPTY)       //비어있는지 확인
            {
                return false;
            }
        }
